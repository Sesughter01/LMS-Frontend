...
import { fetchAllAdmins, fetchAllAdminsBySuper } from "@/store/slices/adminSlice";
import { useDispatch, useSelector } from "react-redux";
import { RootState } from "@/store/store";
import SuperAdminLayout from "@/components/layouts/SuperAdminLayout";
import SuperAdminNav from "@/components/AdminSections/AdminNav/SuperAdminNav";
...
const InstructorPage = () => {
  ...
  const onUpdateAdmin = async () => {
    await dispatch(fetchAllAdminsBySuper());
  };

  const admins = allAdmins;

  useEffect(() => {
    dispatch(fetchAllAdminsBySuper()).then(() => {
    ...
    <>
      <SuperAdminLayout>
        ...
        <div className="mt-[30px]">
          <SuperAdminNav count={admins?.length} setSearchQuery={setSearchQuery} setSortOption={setSortOption} />
        </div>
