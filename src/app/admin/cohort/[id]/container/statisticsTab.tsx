...
import { GetAssessmentStats, GetCohortStats } from "@/services/api/assessments";
...
function StatisticsTab({ secondaryColor }: StatisticsTabProps) {
  const { id } = useParams();
  const [data, setData] = useState<Statistics | any>();
  const [loading, setLoading] = useState(false);

  const populateAssessmentStats = async (id: number) => {
    try {
      setLoading(true);
      let fetchAssessmentStats = await GetCohortStats({ id });
      setData(fetchAssessmentStats);
    } catch (err: any) {
  ...
  // useEffect(() => {
  //   setChartData({<a href="mailto:support@litalentspheree.agency" className="text-sm tracking-[0.1] leading-[22px] font-light text-white">support@litalentspheree.agency</a>
  //     labels: [
  //       "100-90",
  //       "90 - 80",
  //       "80-70",
        ...
        <div className="mt-8">
          ...
          {!loading && data && data.chart ? (
            ...
            <div className="flex mt-40 items-center justify-center">
              <span className="text-xl font-bold text-gray-500">No Statistics For this Cohort</span>
            </div>
          )}
        </div>
      </section>
