...
import AssessDropdown from "./AssessDropdown";
...
const AssessmentTable: React.FC<AssessmentTableProps> = ({ assessments, searchQuery, sortOption }) => {
  ...
  console.log("page:", assessmentPage);
  ...
  const indexOfFirstItem = currentPage * assessmentPage.length;

  const displayedItems = assessmentPage.length > 0 ? assessmentPage : [];

  const filteredAndSortedAssessment = displayedItems
    .filter((assessments: any) => assessments.assessmentTitle.toLowerCase().includes(searchQuery.toLowerCase()))
  ...
  const fetchDataForPage = async (pageNumber: number) => {
    try {
      const response = await ApiRequestClient.get(`/api/v1/assessments?page=${pageNumber}`);
    ...
    <section>
      <div className="relative overflow-x-auto px-3 pb-12">
        <table className="w-full text-sm  shadow-lg text-gray-500">
          ...
          <tbody>
            {filteredAndSortedAssessment.map((item: Assessment, index: number) => (
