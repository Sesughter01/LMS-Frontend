...
export function UploadMe({ onChange, error, isDisabled = false, file }: Props) {
  ...
  useEffect(() => {
    const imageurl = localStorage.getItem("courseImage")
    if (imageurl) {
    ...
    <div className="w-full file-upload-input">
      ...
      {!file && !imageURL ? (
              ...
              <div className="mb-4 flex items-center gap-4">
                ...
                <div>
                  <p className="text-xs text-[#7A7A7A] font-medium">
                    Upload the course cover image here.
                  </p>
                  <p className="text-xs text-[#7A7A7A] font-medium">
        ...
        <div
          ...
          style={{
            backgroundImage: imageURL? `url(${imageURL})`: `url(${URL.createObjectURL(file)})`,
          }}
        >
          {(file || imageURL) && (
...
const Modal = ({
  ...
  return (
    <ReactModal
      // shouldCloseOnEsc={!isUploadingImage}
      // shouldCloseOnOverlayClick={!isUploadingImage}
      isOpen={isOpen}
