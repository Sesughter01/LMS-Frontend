...
const ProgrammeNav: React.FC<OrganizationNavProps> = ({ count, setSearchQuery, setSortOption }) => {
  ...
  return (
    <div className="flex justify-between items-center px-5">
      <div className="flex items-center gap-2">
        ...
        <p className="font-medium text-[#A6A6A6] text-2xl">Programme in total</p>
      </div>
      ...
      <div className="flex items-center gap-4">
        ...
        <button className="flex items-center gap-1 w-full bg-[#1A183E]  p-2 text-white rounded-lg cursor-pointer" onClick={openModal}>
          ...
          <p>Add new Programme</p>
        </button>

        {/* Modal */}
        <ProgrammeModal isOpen={isModalOpen} onClose={closeModal} />
      </div>
    </div>
  );
};
